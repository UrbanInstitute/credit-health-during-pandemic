...
var SELECTED_CAT,
  ...
  DEMOS = ['all', 'comcol', 'white'],
  EXPANDED_DEMOS = ['all', 'white', 'black', 'hisp', 'api', 'native'],
  COUNTY_ID,
  //UPDATE this: include in LATEST_DATA the date for the latest data available. This will inform what data to plot in the map on load.
  LATEST_DATA = '08/01/2021';

var IS_MOBILE = document.body.clientWidth < 800 ? true : false;

var parseTime = d3.timeParse('%m/%d/%Y')

//UPDATE this: add the newest dates to the array dataMonths. It might be the case taht more than one month of data are added in each update
var dataMonths = ['02/01/2020', '04/01/2020', '06/01/2020','08/01/2020','10/01/2020', '12/01/2020', '02/01/2021', '08/01/2021']
...
//UPDATE this, only if researchers include new variables. It gives the proper format to the data points
function formatScore(num){
  if (isNaN(num)) {
    return 'n/a'
  } else if (SELECTED_CAT === 'credscorepos_p50_all'){
    return num
...
var yAxis = d3.axisLeft(y)
  ...
  .tickFormat(function(d, i) {
    ...
    if (SELECTED_CAT === 'credscorepos_p50_all'){
      return d
    } else if (SELECTED_CAT === 'totcollpos_p50_all' || SELECTED_CAT === 'medcollpos_p50_all'){
      return d3.format('$,')(d)
    } else {
...
function dataReady(error, countiesData, statesData, usData, dict, countyLookup, autocompleteSrc, us){
  ...
  var countyList = countyLookup.map(function(d){ return d.id })
   //UPDATE this, only if researchers include new categories. Add new categories to SELECTED_CAT. The second parameter in getQueryParam defines what category is shown by default when loading the feature.
   SELECTED_CAT = getQueryParam('cat', 'subprime_pct_all', ['subprime_pct_all', 'totcollbin_pct_all', 'totcollpos_p50_all', 'credit_ratio_pct_all', 'stud_deliq_60pl_pct_all', 'card_deliq_30pl_pct_all', 'autoret_deliq_60pl_pct_all', 'mortgage_deliq_30pl_pct_all', 'afs_use_bin_pct_all', 'afs_deliquency_bin_pct_all', 'credscorepos_p50_all', 'medcollpos_p50_all', 'medcollbin_pct_all']),
    //UPDATE this. Add new dates to SELECTED_MONTH. The second parameter in getQueryParam, defined at the begining with LATEST_DATA, sets what data should be shown on load.
    SELECTED_MONTH = getQueryParam('month',LATEST_DATA,['02/01/2020', '04/01/2020', '06/01/2020','08/01/2020','10/01/2020', '12/01/2020', '02/01/2021', '08/01/2021']),
  ...
  function filterPlaces(){
    ...
    //UPDATE this, this is a hacky solution that might require more
    // if(SELECTED_MONTH === '08/01/2021' && (SELECTED_CAT === 'afs_use_bin_pct_all' || SELECTED_CAT === 'afs_deliquency_bin_pct_all')) {
    //   placesWithData = []
    // }
  ...
  $('.stateCountySearch').on('select2:unselect', function(evt){
    ...
    if (removedsId.length > 2){
      ...
      //removing a state also removes a county and resets the whole thing, just like zoombtn
  ...
  function updateTitles(){
    ...
    countyName = '',
    countyScore = '',
    usScore = usData[dataMonths.indexOf(SELECTED_MONTH)][SELECTED_CAT]
  ...
  function prepareDataAndUpdateMap(){
    var splitCategory = SELECTED_CAT.split('_'),
    lastPart = splitCategory[splitCategory.length -1]
    var selectedCatWithAllSuffix = SELECTED_CAT.substring(0, SELECTED_CAT.length-lastPart.length) + 'all';
  ...
  function updateLegend(breaks){
    ...
    d3.select('#legend').selectAll('li')
      ...
      .append('li')
      .text(function(d){
        //UPDATE this, if researches add new data in $ or scores that
        if (SELECTED_CAT === 'credscorepos_p50_all'){
          return d
        } else if (SELECTED_CAT === 'totcollpos_p50_all' || SELECTED_CAT === 'medcollpos_p50_all'){
          return d3.format('$,')(d)
  ...
  function checkForData(e){
    ...
    if ( GEOG_LEVEL === 'nation' ){
      ...
      return statesData.filter(function(d){ return d.place === fipsNames[stateFips] })[dataMonths.indexOf(SELECTED_MONTH)][SELECTED_CAT] === '' || statesData.filter(function(d){ return d.place === fipsNames[stateFips] })[dataMonths.indexOf(SELECTED_MONTH)][SELECTED_CAT] === 'n/a'
    } else {
  ...
  function usLineChart(){
    ...
    var topicStub = SELECTED_CAT.substring(0, SELECTED_CAT.length-lastPart.length);

    //2/15 researchers decided they don't want AAPI in this so I've remvoed 'api' from localDemos
    var localDemos = ['all', 'black', 'hisp', 'native', 'white']
    var nested = [];
  ...
  function updateLineLegend(data){
    ...
    var labels = {
      ...
      'state': {
        'all': stateName,
        'comcol': stateName + ' communities of color',
        'white': stateName + ' majority white communities',
        'us-all': 'United States'
      },
      'county': {
        'ct-all': countyInfo,
        'ct-coc': countyInfo + ' communities of color',
        'ct-white': countyInfo + ' majority white communities',
        'us-all': 'United States',
        'st-all': stateName
      }
    }
    ...
    d3.select('#line-legend').selectAll('li')
      ...
      .attr('class', function(d){
        return GEOG_LEVEL + d.key + ' ' + d.class })
  ...
  function fillTemplate(dotData){
    ...
    if ($(window).width() < 800){
      ...
      //changes the month over the whole table
    ...
    var mouseoverData = d3.nest().key(function(d){ return d.date }).entries(dotData);
      var thisMonth = mouseoverData[dataMonths.indexOf(SELECTED_MONTH)].values
  ...
  function drawLine(data){
    ...
    lineXAxis.selectAll('.tick')._groups[0].forEach(function(tick){
      ...
      selection.append('text')
        //UPDATE this, if needed, to avoid text in ticks from overlapping
        .text(function(d) {
          if(d.getFullYear() === 2020 && d.getMonth() === 1) {
            return '2020'
          } else if(d.getFullYear() === 2021 && (d.getMonth() === 1 || d.getMonth() === 7)) {
            return '2021'
          } else if(d.getFullYear() === 2022 && d.getMonth() === 1) {
            return '2022'
          }
        })
      ...
      markers
        ...
        if (IS_MOBILE){
          ...
          lineXAxis.selectAll('.tick').on('click', function(tick){
            ...
            //place the tooltip
            template
              .style('left', tooltipOffset - (template.node().getBoundingClientRect().width / 2) + 19 + 'px')
  ...
  function lilChartByRace(){
    var topicStub = 'credscorepos_p50_';
